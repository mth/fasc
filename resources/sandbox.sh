...
systemd-run -PG --no-ask-password --service-type=exec --unit=${UNIT} \
	...
	-p "Environment=HOME=${HOME} XDG_RUNTIME_DIR=$XDG_RUNTIME_DIR DISPLAY=$DISPLAY WAYLAND_DISPLAY=$WAYLAND_DISPLAY QT_QPA_PLATFORM=wayland-egl QT_WAYLAND_DISABLE_WINDOWDECORATION=1 XDG_SESSION_TYPE=wayland" \
	${COMMAND} "$@"
